...
class AddCoordsNp():
	...
	def call(self, input_tensor):
		...
		print(xx_ones.shape)
		xx_range = np.expand_dims(np.arange(1, self.x_dim+1), 0)
		...
		yy_range = np.expand_dims(np.arange(1, self.y_dim+1), 0)
		...
		xx_channel = xx_channel.astype('float32') / (self.x_dim)
		yy_channel = yy_channel.astype('float32') / (self.y_dim)
...
if __name__ == '__main__':
	coordsOP = AddCoordsNp()
	inputs = np.random.random((1, 64, 64, 3))
	coordsOP.call(inputs)
